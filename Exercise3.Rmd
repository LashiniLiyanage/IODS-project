...
alc %>% summarise(count = n(), mean_grade = mean(G3))

alc %>% group_by(sex) %>% summarize(mean_grade = mean(G3))
...
# tabulate the target variable versus the predictions
tab <- table(high_use = alc$high_use, prediction = alc$prediction)
tab


# tabulate the target variable versus the probabilities of predictions
table(high_use = alc$high_use, prediction = alc$prediction) %>% prop.table()

protab <- prop.table(tab)
protab
